...
class LiveGamesScraper:
    ...
    def is_our_cup(self, match):
        ...
        cup_name = match['cupName'].strip()
        
        # מילות מפתח לזיהוי הגביעים שלנו
        youth_keywords = [
            'נוער',
            'נערים א',
            'נערים ב',
        ...
        # בדוק אם שם הגביע מכיל אחת ממילות המפתח
        for keyword in youth_keywords:
            if keyword in cup_name:
                # נסה למצוא את ה-cupId המתאים
                for cup_id, our_cup_name in OUR_CUPS.items():
                    if keyword in our_cup_name:
                        match['cupId'] = cup_id
                        match['cupName'] = our_cup_name  # עדכן לשם המלא
                        return True
